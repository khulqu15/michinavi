            ...
            <div class="max-w-md w-full p-6 rounded-3xl">
                <form @submit.prevent="submitUserForm" class="text-2xl">
                    ...
                    <div class="form-control space-y-3">
                        ...
                        <input id="email" v-model="email" placeholder="eg. ninno@example.com" type="email" class="input w-full bg-base-100">
...
function submitUserForm() {
    try {
        if(!email.value) {
            ...
            if(emailErrorEl) emailErrorEl.innerHTML = 'Email are required, and must email format'
            return false
        }
        startWebStream()
    } catch (error: any) {
        const emailErrorEl = document.getElementById('email_error') 
        if(emailErrorEl) emailErrorEl.innerHTML = error.message
    }
}
