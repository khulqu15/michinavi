    ...
    <ion-page>
      <ion-content :fullscreen="true">
        <div id="container" class="relative min-h-screen overflow-hidden">
            ...
            <div v-if="cameras.length > 1" class="p-3 absolute top-0 right-0 bg-base-100/30 backdrop-blur-lg inline-block">
                <select v-model="camera" @change="switchCam(camera)" class="select" :class="{'bg-black text-white': theme == 'light', 'bg-white text-black': theme == 'dark'}">
                    <option v-for="item in items" :key="item.deviceId" :value="item.deviceId">
                        {{ item.label }}
                    </option>
                </select>
            </div>
