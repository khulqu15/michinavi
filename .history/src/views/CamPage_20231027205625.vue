<template>
    <ion-page>
      <ion-content :fullscreen="true">
        <div class="fixed w-full h-full flex items-center justify-center bg-base-200 z-20">
            <div class="max-w-md w-full p-6 rounded-3xl">
                <form @submit.prevent="submitUserForm" class="text-2xl">
                    <h2 class="text-4xl">Hi, To Start Demo 
                    ...
                    <div class="form-control space-y-3">
                        ...
                        <input v-model="email" placeholder="eg. ninno@example.com" type="email" required class="input w-full bg-base-100">
                    </div>
                    <ion-button fill="clear" class="btn bg-black text-white mt-6">Demo</ion-button>
                </form>
            </div>
        </div>
        <div>
            <div id="container" class="relative min-h-screen overflow-hidden">
                ...
                <div class="absolute top-0 left-0 p-3 rounded-br-3xl inline-block bg-base-100">
                    <ion-button fill="clear" router-link="/home" router-direction="forward" class="btn rounded-xl btn-sm px-0"
                    :class="{'bg-black text-white': theme == 'light', 'bg-white text-black': theme == 'dark'}">Back</ion-button>
                </div>
                <div v-if="cameras.length > 1" class="p-4 absolute top-0 right-0 bg-base-100/10 rounded-bl-3xl backdrop-blur-lg inline-block">
                    <select v-model="camera" @change="switchCam(camera)" class="select select-sm" :class="{'bg-black text-white': theme == 'light', 'bg-white text-black': theme == 'dark'}">
