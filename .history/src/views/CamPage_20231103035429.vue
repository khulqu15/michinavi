      ...
      <ion-content :fullscreen="true">
        ...
        <div :key="contentKey">
            <div id="container" class="relative min-h-screen overflow-hidden">
                <canvas id="annotations" ref="canvas" class="absolute left-0 top-0 w-full z-20 h-full"></canvas>
...
async function detectObjects() {
    ...
    console.log(myYolo.value)
    if (videoElement && myYolo.value && ctx) {
        boxes.value = await myYolo.value.predict(videoElement, {
            maxBoxes: 5,
            scoreThreshold: .2,
            iouThreshold: .5,
            numClasses: 80,
        ...
        for (const box of boxes.value) {
            ...
            ctx.fillStyle = 'blue';
            ctx.fillText(`${box.class} (${Math.round(box.score * 100)}%)`, box.left + 2, box.top + 10);
        }
    }
}
