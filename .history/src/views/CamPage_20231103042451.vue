...
async function detectObjects() {
  ...
  if (videoElement && myYolo.value && ctx) {
    const boxes = await myYolo.value.predict(videoElement, {
      maxBoxes: 5,
      scoreThreshold: 0.2,
      iouThreshold: 0.5,
      numClasses: 80,
    ...
    for (const box of boxes) {
      if (!detectedObjects.has(box.class)) {
        detectedObjects.add(box.class);
        ctx.strokeStyle = "blue"; // Or any color you prefer
        ctx.fillStyle = "blue";
        ctx.lineWidth = 2;
...
const startWebStream = async() => {
    try {
        ...
        videoElement.value.srcObject = stream.value
        myYolo.value = await yolo.v3tiny();
        resizeCanvas();
        detectObjects();
    } catch(error) {
