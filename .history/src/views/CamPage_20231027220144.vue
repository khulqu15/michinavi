            ...
            <div id="container" class="relative min-h-screen overflow-hidden">
                ...
                <div class="p-4 absolute top-0 right-0 bg-base-100/10 flex items-center gap-2 rounded-bl-3xl backdrop-blur-lg inline-block">
                    <div v-if="cameras.length > 1">
                        ...
                        <ion-button class="select md:select-md select-sm" :class="{'bg-black text-white': theme == 'light', 'bg-white text-black': theme == 'dark'}" onclick="my_modal_2.showModal()">
                            {{ camera.lavbel.length > 10 ? camera.label.substring(0, 10) + '...' : camera.label }}
                        </ion-button>
