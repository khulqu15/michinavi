      ...
      <ion-content :fullscreen="true">
        ...
        <div :key="contentKey">
            <div id="container" class="relative min-h-screen overflow-hidden">
                <div class="absolute w-full top-0 flex z-40 items-center justify-center bg-white p-3">
                    <div id="label-container" ref="labelContainer"></div>
                </div>
                <canvas id="annotations" ref="canvas" class="absolute left-0 top-0 w-full z-30 h-full"></canvas>
...
const modelURL = ref(URL + "model.json");
const metadataURL = ref(URL + "metadata.json");
...
function resizeCanvas() {
    const videoElement = document.getElementById("camera_view") as HTMLVideoElement
    const canvasElement = document.getElementById("annotations") as HTMLCanvasElement
...
const initModel = async () => {
  if (modelURL.value && metadataURL.value) {
    ...
    webcam.value = new tmImage.Webcam(200, 200); // width, height, can set flip to true if needed
    console.log(webcam.value)
    // Prepare the label container
    if (labelContainer.value) {
      labelContainer.value.innerHTML = ""; // Clear the label container
  ...
  if (webcam.value && model.value) {
    ...
    for (let i = 0; i < maxPredictions.value; i++) {
      const classPrediction =
        ...
        if(labelContainer.value) labelContainer.value.childNodes[i].textContent = classPrediction;
    }
    console.log(labelContainer.value)
  }
};
...
const startWebStream = async() => {
    try {
        ...
        videoElement.value.srcObject = stream.value
        await webcam.value.setup({ videoElement: videoElement.value });
