                                                ...
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th class="flex justify-end">
                                                            <ion-button fill="clear" @click="setDelete('all')" class="btn btn-error btn-xs">Delete All</ion-button>
                                                        </th>
                                                    </tr>
                                                </thead>
                                                <tbody>
...
const deleteSelectedItems = async () => {
  const keysToDelete = Object.keys(selectedItems.value).filter((key: any) => selectedItems.value[key]);

  for (const key of keysToDelete) {
    // Delete from Firebase Realtime Database
    const itemRef = dbRef(db, `michiapp/data/${key}`);
    await remove(itemRef);

    // Delete image from Firebase Cloud Storage
    const imagePath = data.value[key].imageFile; // Assuming imageFile contains the path
